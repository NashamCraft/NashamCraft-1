...
execute at @s align y run summon area_effect_cloud ~ ~.2 ~ {Particle:{type:"dragon_breath"},ReapplicationDelay:10,Radius:4f,RadiusPerTick:0f,Duration:600,potion_contents:{custom_effects:[{id:"minecraft:instant_damage",amplifier:3,duration:1}]},Tags:["trueEnding_dragonbreath","trueEnding_dragonbreath_temp"]}
...
execute as @e[distance=0.001..4.0,type=!#true_ending:misc] run damage @s 90 true_ending:mob_attack by @e[type=ender_dragon,limit=1,sort=nearest]
