...
PlayerEvents.loggedIn( event => {
    ...
    if (!event.player.stages.has('curios_slots_added')) {
      ...
      event.server.runCommandSilent("curios remove feet " + player_name)   //server runs a command that adds an aditional belt slot to the new player
      event.server.runCommandSilent("curios remove body " + player_name)   //server runs a command that adds an aditional ring slot to the new player
      event.server.runCommandSilent("curios remove charm " + player_name) //server runs a command that adds an aditional necklace slot to the new player
      event.server.runCommandSilent("curios add bracelet " + player_name)  //List of preset slots at this link https://docs.illusivesoulworks.com/curios/preset-slots
    }
  })